...
func add_conditional_effects(symbol, adjacent):
    symbol.add_effect(effect().if_value_random(1).add_symbol_group("thunder_cloud_rain").animate("shake", 0))
    symbol.add_effect(effect().if_value_random(2).add_symbol_group("thunder_cloud_lightning").animate("shake", 0))
    if values[0] > random(0, 100):
        symbol.add_effect(effect().change_group("thunder_cloud_adds").animate("circle", 0))
